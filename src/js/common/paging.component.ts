...
export class PagingComponent {
  ...
  constructor(pageSize:number, recordCount:number, pageIndex:number, condition:any, callback:Function) {
  ...
  pagerDelegate(obj:any, method:Function, mode:any) : Function {
    return function () {
        var args:any[] = [];
        args.push(mode);
  ...
  ngOnInit(): void {
    ...
    window.onhashchange = function() {
      ...
      var tt = _this.pagerDelegate(_this, _this.callback, { "mode": "nums", "val": parseInt(hashPage) });

      if (_this.flag != true && (("#" + _this.pref + "page" + hashPage == location.hash) || (hashPage == "1" && location.hash == "#"))) {
          tt();
      }
